  ...
  const columns: ColumnDef<BrandResponse>[] = [
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        return <img src={`${IMAGE_URL}/brands/${info.getValue() as string}`} alt={row.name} className="h-10 w-10" />;
      },
    },
    {
      id: "actions",
      header: "عملیات",
      cell: ({ row }) => (
        <Link to={`/admin/brands/update/${row.original.id}`} className="text-yellow-500 dark:text-yellow-300">
          <FaPenClip size={17} />
        </Link>
      ),
    },
