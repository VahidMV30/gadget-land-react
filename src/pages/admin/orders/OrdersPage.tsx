...
import useFetchAllOrdersQuery from "../../../hooks/reactQuery/orders/queries/useFetchAllOrdersQuery";
import useMetadata from "../../../hooks/useMetadata";
import { OrderResponse } from "../../../types/orderTypes";

const OrdersPage = () => {
  useMetadata("سفارشات");
  const { data, isLoading, isError, error } = useFetchAllOrdersQuery();
  ...
  const orderStatusMap: Record<string, { label: string; color: string }> = {
    Pending: { label: "در حال بررسی", color: "text-rose-500" },
    Processing: { label: "در حال پردازش", color: "text-sky-500" },
    Shipped: { label: "ارسال شد", color: "text-green-500" },
  };
  ...
  const columns: ColumnDef<OrderResponse>[] = [
    ...
    {
      id: "orderStatus",
      accessorFn: (row) => orderStatusMap[row.orderStatus]?.label ?? "نامشخص",
      header: "وضعیت سفارش",
      cell: (info) => {
        const label = info.getValue() as string;
        const key = Object.keys(orderStatusMap).find((k) => orderStatusMap[k].label === label);
        const color = key ? orderStatusMap[key].color : "text-gray-500";

        return <span className={`rounded-full px-2 py-1 text-sm font-medium ${color}`}>{label}</span>;
      },
    ...
    },
    {
      ...
      header: "عملیات",
      cell: ({ row }) => (
        <Link to={`/admin/orders/${row.original.id}`} className="text-sky-500">
          <FaEye size={17} />
        </Link>
      ),
    },
