  ...
  const columns: ColumnDef<ProductForAdminTableType>[] = [
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        return <img src={`${IMAGE_URL}/products/${info.getValue() as string}`} alt={row.name} className="h-10 w-10" />;
      },
    },
    {
      accessorKey: "price",
      ...
      cell: (info) => {
        ...
        return discountPrice ? (
          <div className="flex flex-col text-sm">
            <span className="text-red-500 line-through">{price.toLocaleString()}</span>
            <span className="font-semibold text-lime-500">{discountPrice.toLocaleString()}</span>
          </div>
        ) : (
