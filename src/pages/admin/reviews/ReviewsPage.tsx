...
import useFetchAllReviewsQuery from "../../../hooks/reactQuery/reviews/queries/useFetchAllReviewsQuery";
import useMetadata from "../../../hooks/useMetadata";
import { ReviewResponse } from "../../../types/reviewTypes";

const ReviewsPage = () => {
  useMetadata("دیدگاه ها");
  const { data, isLoading, isError, error } = useFetchAllReviewsQuery();
  ...
  const columns: ColumnDef<ReviewResponse>[] = [
    ...
    },
    {
      ...
      cell: (info) =>
        info.getValue() ? (
          <span className="text-green-500">✅ تائید شده</span>
        ) : (
          <span className="text-yellow-500 dark:text-yellow-300">👀 در انتظار تائید</span>
        ),
    },
    {
    ...
    },
    {
      ...
      header: "عملیات",
      cell: ({ row }) => (
        <Link to={`/admin/reviews/${row.original.id}`} className="text-sky-500">
          <FaEye size={17} />
        </Link>
      ),
    },
            ...
            <tbody>
              {data?.length === 0 ? (
                <tr className="odd:bg-gray-100/50 hover:bg-gray-200/50 odd:dark:bg-gray-800/50 dark:hover:bg-gray-700/50">
                  <td colSpan={7} className="animate-pulse truncate px-4 py-3 text-center text-yellow-300">
                    رکوردی یافت نشد!
                  </td>
                </tr>
              ) : (
